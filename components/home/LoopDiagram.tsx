...
/**
 * The 4-step cycle as a clean horizontal diagram. A baby-blue highlight travels
 * the steps in sequence (CSS only). The loop's return is drawn underneath.
 */
export function LoopDiagram({ className }: { className?: string }) {
  return (
    <div className={cn("relative", className)}>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] sm:items-center">
        {STEPS.map((step, i) => (
          <div key={step.title} className="contents">
            <div
              className="assay-step group relative flex flex-col items-center gap-2 rounded-xl border border-border bg-card px-4 py-5 text-center shadow-[0_1px_2px_rgba(15,23,42,0.04)]"
            ...
            {i < STEPS.length - 1 && (
              <ArrowRight
                className="mx-auto hidden size-4 shrink-0 text-[#9cc4ea] sm:block"
                strokeWidth={2}
              />
            )}
          </div>
      ...
      <div className="mt-3 flex items-center justify-center gap-2 font-mono text-[11px] text-muted-foreground/70">
        <RefreshCw className="size-3 text-[#9cc4ea]" />
